...
export class BlobStore implements ConversationReferenceStore {
  ...
  async list(
    ...
    const result: Partial<ConversationReference>[] = [];
    const iterator = this.client
      .listBlobsFlat()
      .byPage({ maxPageSize: pageSize, continuationToken });
    const response: ContainerListBlobFlatSegmentResponse = (
      await iterator.next()
    ).value;
  ...
  // =============================
  /**
   * Retrieves the stored conversation reference for the given user ID.
   * Assumes that the key used when adding is the user ID (or derived from it).
   */
  async getConversationReferenceByUserId(
    userId: string
  ...
  // A helper method to normalize key to meet Azure Blob naming requirement
